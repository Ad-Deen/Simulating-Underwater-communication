...
def ambient_noise(freq,shipping_crowd = 0.2):
    ...
    s = shipping_crowd
    w = 6.47
    noise_wind = 50+7.5*w**(0.5)+20*math.log(freq)-40*math.log(freq+0.4)
    noise_turbulent = 17-30*math.log(freq)
    noise_surface_wave = 40 + 20*(s - 0.5) + 26*math.log(freq) - 60*math.log(freq + 0.03)
    # if freq > 100e3:
    noise_thermal = -15 + 20*math.log(freq)
    # else:
    #     noise_thermal = 0.0
    return (noise_wind + noise_turbulent + noise_surface_wave + noise_thermal)

def add_noise_to_data(data,source_signal,tx_freq,noise_amp):
    '''
    Arguments taken :
        data = the x_plot array of distances covered
        source_signal = The Initial source signal on y_plot which will be modified for introducing pathloss and ambient noise
        tx_freq = Operating frequency of the source
    Output:
        - An array of attenuated and noisy signal based on given frequency and distance
    '''
    ...
    while True:
        ...
        # Fill the result array by multiplying each element with the attenuating factor
        for i in range(len(data)):
            result_data[i] = source_signal[i]*(1/(pathloss(tx_freq,data[i],depth_calc(source_pos,reciever_pos))))

        ambient = ambient_noise(tx_freq) * noise_amp
        # Generate random noise for the data
        noise = np.random.uniform(-ambient, ambient, len(data))
        data_with_noise = result_data + noise
...
tx_freq = 4000 # Hz transmitter frequency
...
distance_grid = np.arange(0.0,  distance_x, grid_resolution )
time_grid = np.arange(0.0 , distance/sound_speed , (distance/sound_speed)/len(distance_grid))

# for i in range(len(time_grid)):
#     result_data[i] = source_signal[i]*(1/(pathloss(tx_freq,data[i],depth_calc(source_pos,reciever_pos))))

y_data1 = source_amp * np.sin(2*np.pi*tx_freq*time_grid)
y_data2 = source_amp * np.sin(2*np.pi*tx_freq*time_grid)

line1, = ax1.plot(time_grid, y_data1)
line2, = ax2.plot(distance_grid, y_data2)

# Create threads for adding noise to the data for both graphs
noise_gen1 = add_noise_to_data(distance_grid,y_data1,tx_freq,1)
noise_thread1 = threading.Thread(target=lambda: next(noise_gen1))
noise_thread1.daemon = True
noise_thread1.start()

noise_gen2 = add_noise_to_data(distance_grid,y_data2,tx_freq,noise_amplifier)
noise_thread2 = threading.Thread(target=lambda: next(noise_gen2))
noise_thread2.daemon = True
noise_thread2.start()

# Set x-axis limits
ax1.set_xlim(0, distance)  # You can adjust the limits based on your data range
ax2.set_xlim(0, distance)
ax1.set_xlabel('Time (s)')
ax1.set_ylabel('Source Strength (MicroPascal/meter)')
ax1.set_title('Transmitted Signal')
ax2.set_xlabel('Distance Covered (m)')
ax2.set_ylabel('Recieved Signal Strength (MicroPascal/meter)')
ax2.set_title('Recieved Signal with Pathloss and Ambient noise')
